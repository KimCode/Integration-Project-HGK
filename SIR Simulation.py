# Setup:
# * Create window (Left = Simulation, Right = Graph).
# * Create a population as a list of Human objects.
# * Create 3 lists to store daily S, I, and R counts:
# susceptible_data, infected_data, recovered_data
# * Set the first person as "Infected" (patient zero).

# Main loop:
# * For each person:
# * Update position (x + dx, y + dy).
# * Bounce if hitting simulation boundaries.
# * If status is "Infected" and infected long enough:
# * Change status to "Recovered".

# * Make a list of infected people and a list of susceptible people.
# * For each infected person:
# * For each susceptible person:
# * If distance < infection radius:
# * If random chance < infection rate:
# * Change susceptible person to "Infected".
# * Set infection_timer = current frame.

# Data tracking:
# * Every 60 frames (= 1 simulated day):
# * Count total susceptible, infected, and recovered people.
# * Append each count to its corresponding list.
# * Increase day by 1.

# Visualization:
# * Draw moving dots in simulation area.
# * Draw SIR graph using stored daily data.
# * Show title, live S/I/R counts, current day, and legend.
# * Stop updating once the simulation reaches the day cap.
